...
def sanitize_windows_filename(file_name: str) -> str:
    """
    Limpia nombres inválidos para Windows.
    Mantiene el nombre original siempre que sea posible.
    """
    ...
    # Windows no permite nombres terminados en punto o espacio.
...
def get_original_file_name_from_message(message) -> Optional[str]:
    """
    Extrae el nombre original que Telegram trae en DocumentAttributeFilename.
    En tu caso:
    3913267432197156605.mp4
    """
...
def get_preferred_final_file_name(message) -> str:
    """
    Prioridad:
    1. Nombre original de Telegram: DocumentAttributeFilename.
    2. message_id + extensión detectada.
    """
    original_file_name = get_original_file_name_from_message(message)
